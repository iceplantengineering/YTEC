...
| レベル | 内容 | 判定 |
|--------|------|------|
| **Level 1: STロジック単体** | D339/D340書込→D330自動判定 | ✅ **対応済** |
| **Level 2: ハンドシェイク単体** | M1057等の条件成立時→D212/D214遷移 | ✅ **対応済** |
| **Level 3: 2PLC間連動** | B01→SRMのタスク伝達・完了通知 | ✅ **対応済** (U2\Gブロックコピー実装済) |
| **Level 4: CV含む4PLC連動** | CV→B01のBデバイス同期を含む全フロー | ✅ **対応済** (Bデバイスコピー実装済) |
| **Level 5: Lua結合テスト** | TTask.lua → Redis → Modbus → GATEWAY → GX Sim → 全PLC連動 | ✅ **対応済** (接続先変更マニュアル作成済) |

---

## 6. 対応済みの改修内容と今後のアクション

**【すでに完了した改修・整備（上記のギャップに対する解決策）】**
1. **cc_link_copy.pyの拡張完了**: Bデバイス（在席・リフタ状態等）とU2\G代替ブロック（D200〜D225 → D3000〜D3025）のコピー設定を `config.json` に追加済みです。
2. **plc_access_mx.pyのデバイス種別対応完了**: Dレジスタだけでなく、B接点、M接点、U2\G等、あらゆる文字列表現のデバイス仕様に対応するよう改良済みです。
3. **Lua接続先の変更手順書作成完了**: `DOC3/lua_connection_update_guide.md` にて、`192.168.1.x:502` を `127.0.0.1:1502x` に変更する手順を整備済みです。

**【実際にテストを行う前の最終アクション（ユーザー作業）】**
1. `DOC3/lua_connection_update_guide.md` に従い、お手元のLuaファイル内のIPアドレスを書き換える。
2. MX Component通信設定ユーティリティで、各PLC（論理局番1001～1004）の設定をSimulator用に行う。
3. GX Simulator2 を4台起動する。
4. GATEWAY（`main.py`）を起動し、Redis経由でLuaタスクを流し込む。

以上により、実機と全く同じ機能を保ったまま、PC1台で完璧なフルシミュレーションテストを実行できる状態に到達しています。
